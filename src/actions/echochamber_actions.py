...
def post_echochambers(agent, **kwargs):
    ...
    if (
        current_time - agent.state["echochambers_last_message"]
        > agent.echochambers_message_interval
    ):
        agent.logger.info("\n📝 GENERATING NEW ECHOCHAMBERS MESSAGE")

        # Generate message based on room topic and tags
        previous_messages = agent.connection_manager.connections[
            "echochambers"
        ].sent_messages
        previous_content = "\n".join(
            [f"- {msg['content']}" for msg in previous_messages]
        )
...
def reply_echochambers(agent, **kwargs):
    ...
    history = agent.connection_manager.perform_action(
        connection_name="echochambers", action_name="get-room-history", params={}
    )
    ...
    if history:
        ...
        for message in history:
            ...
            if sender_username == agent.connection_manager.connections[
                "echochambers"
            ].config["sender_username"] or message_id in agent.state.get(
                "echochambers_replied_messages", set()
            ):
                agent.logger.info(
                    f"Skipping message from {sender_username} (already replied or own message)"
                )
                continue
            ...
            username_prompt = (
                f"Refer the sender by their @{sender_username}"
                if refer_username
                else "Respond without directly referring to the sender"
            )

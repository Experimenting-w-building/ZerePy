...
class EchochambersConnection(BaseConnection):
    def __init__(self, config: Dict[str, Any]):
        ...
        if not all(
            [
                ...
                self.sender_model,
                self.history_read_count,
                self.post_history_track,
            ]
        ):
            missing = [
                k
            ...
            raise EchochambersConfigurationError(
    ...
    def validate_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if (
            not isinstance(config["history_read_count"], int)
            or config["history_read_count"] <= 0
        ):
    ...
    def get_room_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            room_info = next(
                (room for room in response.get("rooms", []) if room["id"] == self.room),
                None,
            )
            if not room_info:
    ...
    def get_room_history(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            return [
                ...
                for msg in messages[: self.history_read_count]
    ...
    def send_message(self, content: str) -> Dict[str, Any]:
        ...
        try:
            ...
            self.sent_messages.append({"content": content, "timestamp": time.time()})
        ...
        try:
            ...
            for message in reversed(history):
                ...
                if (
                    ...
                    and message["sender"]["username"] != self.sender_username
                ):
    ...
    def _log_metrics(self) -> None:
        ...
        if current_time - self.metrics["last_metrics_log"] >= 300:
            ...
            success_rate = (
                (self.metrics["messages_sent"] / total_attempts * 100)
                if total_attempts
                else 0
            )
            avg_latency = (
